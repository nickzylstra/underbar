(function() {
  ...
  _.uniq = function(array, isSorted, iterator = _.identity) {
    ...
    if (isSorted) {
      ...
      _.each(array, function(el) {
        const iterEl = iterator(el);
        if (!prevEls.hasOwnProperty(iterEl)) {
          prevEls[iterEl] = true;
          uniqArray.push(iterEl);
        }
      });
  ...
  _.reduce = function(collection, iterator, accumulator) {
    ...
    _.each(collection, function(item) { // if removing if statement from loop is desired, reimplement each with but adjusting start position
      if (accUndefined) {
        accumulator = item;
        accUndefined = false;
      } else {
